...
async function handleCotizar(env, input) {
  const { beginDate, endDate, destinationCode, passengers } = input;

  if (!beginDate || !endDate || !destinationCode || !passengers?.length) {
  ...
  const body = {
    ...
    destinationCode: Number(destinationCode),
  ...
  return acFetch(env, "/api/Quote", body);
}
...
async function handleLinkPago(env, input) {
  const { counterCode, productCode, rateCode, beginDate, endDate,
          destinationCode, passenger } = input;

  if (!counterCode || !productCode || !rateCode || !passenger?.email) {
  ...
  const body = {
    ...
    rateCode,
    paymentDetails: {
      currency: "ARS",   // Argentina → ARS (ver tabla de gateways en la doc)
    },
  };

  const result = await acFetch(env, "/api/Issuance/payment-button/vouchers", body);
  ...
  if (result.status === 200 && result.data) {
    const paymentUrl =
      ...
      result.data?.paymentDetails?.url ||
      result.data?.paymentDetails?.paymentUrl;
    return { status: 200, data: { paymentUrl, raw: result.data } };
  }
  return result;
}
